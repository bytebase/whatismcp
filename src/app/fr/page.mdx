...
- **Ressources** : Objets de données qui peuvent être référencés et récupérés. Cela inclut les documents, les images, les schémas de base de données et autres données structurées.
...
- **Outils** : Fonctions qui peuvent être exécutées par le modèle de langage pour effectuer des actions comme interroger une base de données, appeler une API ou traiter des données.
...
| ------------- | ------------------------------------------------------------------------- |
| **Ressources** | Informations de schéma extraites de la base de données |
| **Prompts** | Prompts spécifiques au domaine des bases de données qui aident le modèle à générer du SQL correct |
| **Outils** | Exécuter des commandes SQL contre la base de données |
