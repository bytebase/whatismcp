...
- **리소스**: 참조하고 검색할 수 있는 데이터 객체. 문서, 이미지, 데이터베이스 스키마 및 기타 구조화된 데이터가 포함됩니다.
- **프롬프트**: 특정 작업이나 도메인에 최적화된, 언어 모델과의 효과적인 상호작용을 생성하기 위한 템플릿.
- **도구**: 데이터베이스 쿼리, API 호출, 데이터 처리와 같은 작업을 수행하기 위해 언어 모델이 실행할 수 있는 함수.
...
| ------------- | ------------------------------------------------------------------------- |
| **리소스** | 데이터베이스에서 가져온 스키마 정보 |
| **프롬프트** | 모델이 올바른 SQL을 생성하도록 돕는 데이터베이스 도메인 특화 프롬프트 |
| **도구** | 데이터베이스에 대해 SQL 명령 실행 |
