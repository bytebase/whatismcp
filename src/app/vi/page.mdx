...
- **Tài nguyên (Resources)**: Các đối tượng dữ liệu có thể được tham chiếu và truy xuất. Chúng bao gồm tài liệu, hình ảnh, schema cơ sở dữ liệu và dữ liệu có cấu trúc khác.
- **Prompts**: Các mẫu để tạo ra tương tác hiệu quả với mô hình ngôn ngữ, được tối ưu hóa cho các tác vụ hoặc lĩnh vực cụ thể.
- **Công cụ (Tools)**: Các hàm có thể được thực thi bởi mô hình ngôn ngữ để thực hiện các hành động như truy vấn cơ sở dữ liệu, gọi API hoặc xử lý dữ liệu.
...
| ------------- | ------------------------------------------------------------------------- |
| **Tài nguyên** | Thông tin schema được lấy từ cơ sở dữ liệu |
| **Prompts** | Các prompt dành riêng cho lĩnh vực cơ sở dữ liệu giúp mô hình tạo SQL chính xác |
| **Công cụ** | Thực thi các lệnh SQL đối với cơ sở dữ liệu |
