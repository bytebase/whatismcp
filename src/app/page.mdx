...
> **Note:** For the detailed explanation, check the [official MCP documentation](https://modelcontextprotocol.io/docs/concepts/architecture).
...
<Image
  src={mcpArchitecture}
  alt="MCP Architecture diagram showing the relationship between hosts, clients, servers, and data sources"
  width={700}
  height={400}
/>
...
Let's demonstrate this by using the classic AI use case, Text-to-SQL. Text-to-SQL translates natural language to SQL. A typical implementation:

1. SQL client fetches the schema from the target databases
2. SQL client sends LLM a prompt containing the schema, and LLM replies with the SQL
3. SQL client executes the SQL against the target database

Before MCP, every SQL client needs to implement this for every supported database. With MCP, the SQL Client just needs to implement the
MCP client protocol and each database vendor just needs to implement a MCP server once.

Essentially, it turns an M×N problem into an M+N solution, by abstracting with another level of indirection.
...
<Image
  src={xkcdStandards}
  alt="XKCD comic #927: Standards - How standards proliferate"
  width={550}
  height={300}
/>
...
<Image
  src={mcpMembers}
  alt="Model Context Protocol working group members"
  width={650}
  height={350}
/>
...
From the technical perspective, MSP mostly resembles LSP which has achieved great success in its domain. In the long term, MSP could be as important as SQL to become the lingua franca in the AI-native era.
